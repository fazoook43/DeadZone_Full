...
resolve_final_zip_name() {
  ...
  if [[ -n "$custom" ]]; then
    ...
    case "$preset" in
      DeadZone_Gaming_V1|DeadZone_EPiC_V1) chosen="$preset" ;;
      custom|"") chosen="${BUILD_NAME}_${DEVICE_CODENAME}_fastboot" ;;
      *)         chosen="$preset" ;;
    esac
  fi
...
final_zip_name() {
  [[ -z "${FINAL_ZIP_NAME:-}" ]] && resolve_final_zip_name
  printf '%s\n' "$FINAL_ZIP_NAME"
}
...
resolve_rom_region() {
  ...
  case "$requested" in
    China|china|CN|cn)       ROM_REGION="China"  ;;
    Global|global|GL|gl)     ROM_REGION="Global" ;;
    India|india|IN|in)       ROM_REGION="India"  ;;
    auto|Auto|AUTO|"")
      if   printf '%s\n' "$text" | grep -qiE 'CNXM|[^A-Z]CN[^A-Z]|China';       then ROM_REGION="China"
      elif printf '%s\n' "$text" | grep -qiE 'INXM|[^A-Z]IN[^A-Z]|India';       then ROM_REGION="India"
      elif printf '%s\n' "$text" | grep -qiE 'MIXM|GLXM|Global|GLOBAL|EUXM|EEA'; then ROM_REGION="Global"
      else
        ROM_REGION="Global"
        warn "Could not auto-detect ROM region; defaulting to Global"
      fi ;;
...
prepare_env() {
  ...
  rm -rf "$INPUT" "$EXTRACTED" "$OUTPUT" "$OUTPUT_FINAL" \
         "$WORKSPACE/rom_info.env" "$WORKSPACE/partitions.tsv"
...
load_device_profile() {
  ...
  local profile="$DEVICES/$codename.conf"
  [[ -f "$profile" ]] || return 1
  ...
  if [[ -z "${DYNAMIC_PARTITIONS:-}" && \
        "$(declare -p SUPER_PARTITIONS 2>/dev/null)" =~ "declare -a" ]]; then
...
fetch_rom() {
  ...
  log "Downloaded: $(basename "$ROM_FILE") ($(human_size "$ROM_FILE"))"
...
detect_rom_version() {
  ...
  local version=""
  version="$(printf '%s\n' "$source_text" \
    | grep -oE 'OS[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\.[A-Z0-9]+' | head -n 1 || true)"
  if [[ -z "$version" && -n "${ROM_FILE:-}" ]]; then
    version="$(basename "$ROM_FILE" \
      | grep -oE 'OS[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\.[A-Z0-9]+' | head -n 1 || true)"
  fi
...
# ---------------------------------------------------------------------------
# try_load_or_generate_device_profile
#
#  Wraps the old "load_device_profile || die" pattern.
#  New flow:
#    1. Try to load existing conf
#    2. If missing → run device_probe → try generate → try load again
#    3. If still missing → die with helpful message
# ---------------------------------------------------------------------------
try_load_or_generate_device_profile() {
  ...
  log "Device profile not found for '$DEVICE_CODENAME' — running auto-probe …"

  # Run probe strategies that don't need extraction yet
  # (build.prop strategy runs later, after extract_payload_all_images)
  _probe_from_filename        2>/dev/null || true
  _probe_from_ota_metadata    2>/dev/null || true
  probe_super_metadata        2>/dev/null || true

  # Try to generate a conf from probed data
  if generate_device_conf; then
    # Reload
    load_device_profile && return 0
  fi

  # Still nothing — fail clearly
  die "Missing device profile: devices/${DEVICE_CODENAME}.conf
No auto-generated profile could be created (SUPER_SIZE or GROUP_SIZE unknown).
Run lpdump on the stock super.img and create the profile manually, or
set SUPER_SIZE / DYNAMIC_PARTITION_GROUP_SIZE as environment variables."
}
