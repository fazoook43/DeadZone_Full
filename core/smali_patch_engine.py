...
class SmaliPatchEngine:
    engine_type = "smali"

    def validate_patch(self, patch: dict[str, Any]) -> list[str]:
        errors = []
        if patch.get("type") not in (None, "smali"):
            errors.append(f"{patch.get('id', 'patch')}: expected smali patch type")
    ...
    def prepare_workspace(self, target: dict[str, Any], workspace: str | Path) -> dict[str, Any]:
        path = Path(workspace) / "smali" / target["id"]
    ...
    def applicability(self, target: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "reason": "target resolved" if target.get("applicable") else "target container not found",
        }
